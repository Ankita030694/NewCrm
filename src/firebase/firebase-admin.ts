...
if (!defaultApp) {
  try {
    ...
    if (process.env.FIREBASE_PROJECT_ID && process.env.FIREBASE_CLIENT_EMAIL && process.env.FIREBASE_PRIVATE_KEY) {
      ...
      console.warn("Debug: PROJECT_ID present?", !!process.env.FIREBASE_PROJECT_ID);
      console.warn("Debug: CLIENT_EMAIL present?", !!process.env.FIREBASE_CLIENT_EMAIL);
      console.warn("Debug: PRIVATE_KEY present?", !!process.env.FIREBASE_PRIVATE_KEY);
    }
  } catch (error) {
    console.error("[Firebase Admin] Failed to initialize default app (CRM):", error);
  }
} else {
  console.log("[Firebase Admin] Default app (CRM) already initialized.");
}

// Export auth and storage for CRM (Default)
// These will use the default app initialized above
const defaultAppInstance = getApps().find(app => app.name === '[DEFAULT]');
export const auth = defaultAppInstance ? admin.auth() : null;
export const storage = defaultAppInstance ? admin.storage() : null;
export const db = defaultAppInstance ? admin.firestore() : null;

// Legacy aliases for CRM (to maintain compatibility with existing code)
export const adminAuth = auth;
export const adminStorage = storage;
export const adminDb = db;
export const adminMessaging = defaultAppInstance ? admin.messaging() : null;
