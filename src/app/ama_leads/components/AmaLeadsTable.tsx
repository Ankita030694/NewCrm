...
const AmaLeadsTable = (props: LeadsTableProps) => {
  ...
  const getAssignmentOptions = () => {
    if (currentUserRole === "admin" || currentUserRole === "overlord") {
      ...
      return dynamicSalesTeamMembers.filter((member) => member.role === "sales" || member.role === "salesperson")
    }

    if (currentUserRole === "sales" || currentUserRole === "salesperson") {
      // Sales can only assign to themselves
      return dynamicSalesTeamMembers.filter(
        (member) => member.name === currentUserName && (member.role === "sales" || member.role === "salesperson"),
      )
    }
