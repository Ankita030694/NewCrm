...
const AmaLeadRow = ({
  ...
  const debtDisplay =
    debtRaw !== null && debtRaw !== undefined && debtRaw !== "" ? `₹${Number(debtRaw).toLocaleString("en-IN")}` : "N/A"

  // Show source from the 'source' field only and assign colors per source
  const sourceKey = sourceRaw.trim().toLowerCase()
  const isCS =
    sourceKey === "credsettle" ||
    sourceKey === "credsettlee" ||
    sourceKey === "credsettle " ||
    sourceKey === "credsettle contact" ||
    sourceKey === "credsettle form" ||
    sourceKey === "credsettle form"
      ...
      <tr className={`transition-colors duration-150 ${rowColors.rowBg} ${!canEdit ? "opacity-100" : ""}`} role="row">
        ...
        {columnVisibility.checkbox && (
          <td className="px-5">
            ...
            {!canEdit && (
              <div
                className="mt-1 text-xs text-[#D2A02A] flex items-center"
                title={
        ...
        {columnVisibility.date && (
          <td className="px-0.5 py-0.5 w-20">
            <div className="flex flex-col gap-0.5">
              ...
              <span
                className={`text-[10px] leading-tight ${rowColors.textColor ? "text-[#ffffff]/70" : "text-[#5A4C33]/70"}`}
              >
                {time}
              </span>
            </div>
        ...
        {columnVisibility.name && (
          <td className="px-1 max-w-[200px]">
            <div className="flex flex-col gap-0.5">
              <div
                className={`font-medium flex items-center text-[16px] px-5 ${rowColors.textColor || "text-[#5A4C33]"}`}
              >
                {name}
              ...
              <div className="flex items-center text-[10px] px-5">
                <a
                  href={`mailto:${email}`}
                  className={`hover:underline truncate max-w-[180px] ${rowColors.textColor || "text-[#D2A02A]"}`}
                >
                  {email}
                </a>
              </div>
          ...
          <td className="px-1 py-0.5 text-[11px] px-5">
            <div className="space-y-1">
              <div>
                ...
                <span className={rowColors.textColor || "text-[#5A4C33]"}>{debtDisplay}</span>
              </div>
            </div>
          </td>
        )}
                ...
                placeholder={
                  !canEdit
                    ...
                    : notesValue && notesValue.trim() !== ""
                      ? ""
                      : lead.lastNote || lead.salesNotes || "Add sales notes..."
                }
              />
          ...
          <div className="fixed inset-0 bg-[#5A4C33]/50 flex items-center justify-center z-50">
            <div className="bg-[#ffffff] rounded-xl p-6 w-full max-w-2xl max-h-[80vh] border border-[#5A4C33]/20 shadow-2xl">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-xl font-semibold text-[#5A4C33]">Customer Query - {name}</h3>
                <button
