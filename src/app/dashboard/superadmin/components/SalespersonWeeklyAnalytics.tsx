...
export const SalespersonWeeklyAnalyticsComponent: React.FC<SalespersonWeeklyAnalyticsProps> = ({
    ...
    // Sort data by total amount across selected months (Descending)
                        ...
                        <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                            {sortedData.map((item) => (
                                <tr key={item.salespersonName} className="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                                    ...
                                    {displayedMonths.map(month => {
                                        // If it's the current month, we might use monthlyTotal/weeks OR history[month]
                                        // The action ensures history[month] contains the same data.
