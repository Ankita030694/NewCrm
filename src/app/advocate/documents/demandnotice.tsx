...
export default function DemandNoticeForm({ onClose }: DemandNoticeFormProps) {
  ...
  const getBankOptions = () => {
    ...
    Object.entries(bankGroups).forEach(([bankName, accounts]) => {
      ...
      const displayBankName = matchedBank || bankName;
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          ...
          />
          {selectedBank && (bankData[selectedBank.split('|')[0]] || findClosestBankMatch(selectedBank.split('|')[0], Object.keys(bankData))) && <p className="text-xs text-gray-500 mt-0.5">Auto-filled from selection (bank name corrected to database format)</p>}
          {selectedBank && selectedBank.includes('|') && !bankData[selectedBank.split('|')[0]] && !findClosestBankMatch(selectedBank.split('|')[0], Object.keys(bankData)) && <p className="text-xs text-yellow-500 mt-0.5">⚠️ Bank not in system - please enter manually</p>}
        </div>
